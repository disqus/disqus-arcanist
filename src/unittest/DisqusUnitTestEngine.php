...
class DisqusUnitTestEngine extends ArcanistBaseUnitTestEngine {
    ...
    private function runTestSuite($project_root, $js=false) {
        ...
        $args = array($runtests_command, $xunit_path, $coverage_path, implode(',', $paths));
        $num_args_used = preg_match_all('/[^\\%]\\%s/', $runtests_command, $matches);
        $args = array_slice($args, 0, $num_args_used + 1);
        $exec_future_reflect = new ReflectionClass('ExecFuture');
        $future = $exec_future_reflect->newInstanceArgs($args);
        $future->setCWD($project_root);
        try {
            $future->resolvex();
